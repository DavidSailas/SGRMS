...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stud_status = 1;
    $stud_sql = "INSERT INTO students (id_num, suffix, lname, fname, mname, sex, bod, address, mobile_num, email, educ_level, year_level, section, program, s_image, previous_school, status, parent_id)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stud_stmt = $conn->prepare($stud_sql);
    $stud_stmt->bind_param(
        "sssssssssssssssiii",  
        $id_num, $suffix, $lname, $fname, $mname, $sex, $bod,
        $address, $mobile_num, $email, $educ_level, $year_level, $section, $program,
        $image_path, $previous_school, $stud_status, $parent_id
    );
